...
void guardarInventario(Inventario* inventario) {
    FILE* archivo = fopen(FILENAME, "a"); // Cambiado a "a" para agregar al archivo
    if (archivo == NULL) {
...
void cargarInventario(Inventario* inventario) {
    ...
    for (int i = 0; i < MAX_GENEROS; i++) {
        if (fscanf(archivo, "%*s %d %d %d", &inventario[i].vinilos, &inventario[i].cds, &inventario[i].cassettes) != 3) {
